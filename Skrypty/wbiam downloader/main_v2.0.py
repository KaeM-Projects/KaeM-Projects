...
def copy_all(src_directory,dst_directory):
    ...
    dst_directory = os.path.normpath(dst_directory)
    ...
    for i in tqdm(range(len(file_list)), desc="Files copying: ", bar_format="\033[1;31m{desc} [{bar}]\033[m {percentage:3.0f}%|{n_fmt}/{total_fmt} [{elapsed}<{remaining}] | IPS: {rate_fmt}"):
        ...
        if os.path.exists(dst_file_path):
            ...
            if os.path.isdir(src_file_path):
                ...
                if os.path.exists(os.path.dirname(dst_file_path)):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog='wbijam.pl downloader script options',
        description='choose what to do when finished',
        epilog='')
    parser.add_argument('-hbr', '--hibernate', action='store_true')

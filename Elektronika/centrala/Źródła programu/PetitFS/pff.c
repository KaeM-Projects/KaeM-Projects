...
FRESULT file_properties(DWORD* size, DWORD* carriage, DWORD* cluster0, DWORD* cluster_act, DWORD* sector ){
	...
	*carriage = fs->fptr;		/* File R/W pointer */
	*size = fs->fsize;		/* File size */
	*cluster0 = fs->org_clust;	/* File start cluster */
	*cluster_act = fs->curr_clust;	/* File current cluster */
	*sector = fs->dsect;		/* File current data sector */
	return FR_OK;
}
...
)
{
	...
	*line_cnt = 0;
	...
	while(!eol){
		if((fs->fsize - fs->fptr)<=buffer_size){
			btr = (WORD)(fs->fsize - fs->fptr);
			eol = 1;
			*eof = 1;
		}
		else btr = buffer_size;
		...
		while (btr)	{									/* Repeat until all data transferred */
			...
			rcnt = 512 - ((WORD)fs->fptr % 512);		/* Get partial sector data from sector buffer */
			if (rcnt > btr)rcnt = btr;
			dr = disk_readp(!buff ? 0 : p_i_buff, fs->dsect, (WORD)(fs->fptr % 512), rcnt);
			if (dr) goto fr_abort;
			btr -= rcnt;

			for(BYTE i=0; i < rcnt; i++){
				if(i_buff[i] == 13){
					eol = 1;
					if(fs->fptr > 2) *eof = 0;
					if((rcnt-i)>=2)fs->fptr+=2;
					else{
						fs->fptr++;
						plus_one = TRUE;
					}
